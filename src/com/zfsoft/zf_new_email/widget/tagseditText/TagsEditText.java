 ...
 * 
 */
package com.zfsoft.zf_new_email.widget.tagseditText;
...
public class TagsEditText extends AutoCompleteTextView {
	...
	private String[] getStringNoSame(String[] tags) {
		if (tags == null) {
			return null;
		}
	...
	public void setTagsTextColor(@ColorRes int color) {
		...
		mTagsTextColor = getResources().getColor(color);
	...
	private void init(@Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		...
		if (attrs == null) {
			...
			mTagsTextColor = getResources().getColor(R.color.defaultTagsTextColor);
			mTagsTextSize = ResourceUtils.getDimensionPixelSize(context, R.dimen.font_small);
			...
			mDrawablePadding = ResourceUtils.getDimensionPixelSize(context, R.dimen.item_email_list_margin);
		} else {
			...
			try {
				...
				mTagsTextColor = typedArray.getColor(R.styleable.TagsEditText_tagsTextColor, getResources().getColor(R.color.defaultTagsTextColor));
				mTagsTextSize = typedArray.getDimensionPixelSize(R.styleable.TagsEditText_tagsTextSize, ResourceUtils.getDimensionPixelSize(context, R.dimen.font_small));
	...
	private void setTags() {
		...
		if (mLastString.endsWith(SEPARATOR) && !str.endsWith(NEW_LINE) && isDeleting && !mTagSpans.isEmpty()) {
	...
	private void updateTags(String newString) {
		...
		if (!TextUtils.isEmpty(source) && !source.equals(NEW_LINE)) {
			boolean isSpan = source.endsWith(NEW_LINE) || (!mIsSpacesAllowedInTags && source.endsWith(SEPARATOR));
			if (isSpan) {
	...
	private void removeTagSpan(Editable editable, TagSpan span, boolean includeSpace) {
		...
		if (mListener == null)
			return;
		mListener.onTagsChanged(convertTagSpanToList(mTagSpans));
		mListener.onItemRemoved(tagIndex);
	}
