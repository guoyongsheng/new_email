...
public class EmailDetailActivity extends BaseActivity implements OnClickListener {
	...
	// webview是否可以返回
	private boolean webViewCanGoBack(EmailDetailFragment fragment) {
		if (fragment == null) {
			return false;
		}

		return fragment.webViewCanGoBack();
	}
	...
	private EmailDetailFragment getFragment() {
		...
		if (adapter != null) {
			...
			if (fragment != null && fragment instanceof EmailDetailFragment) {
				return (EmailDetailFragment) fragment;
			}
		}
