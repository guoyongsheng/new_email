 ...
 * @author wesley
 * @date: 2017-1-19
 * @Description: oa邮箱的接口实现
 */
public class EmailSendOrReplyInOAServiceImpl extends WebServiceUtil implements IEmailSendOrReplyInOAService {

	private static final String TAG = "EmailSendOrReplyInOAServiceImpl";
	private CallBackListener<Boolean> listener;

	@Override
	public void sendEmailInOA(String emailId, String receiverId, String receiverName, String ccId, String ccName, String subject, String content, int mailType, String url, String app_token, CallBackListener<Boolean> listener) {
		...
		try {
			...
			paramList.add(new DataObject("type", CodeUtil.encode(mailType + "", app_token)));
			...
			e.printStackTrace();
			Logger.print(TAG, "oa邮件发送失败 失败信息是:" + e.getMessage());
		}

		asyncConnect(NAMESPACE_OA, FUN_EMAIL_SENDMAIL, url, paramList, BaseApplication.getInstance());
	}
	...
	public void taskexecute(String response, boolean bTimeOut) throws Exception {
		...
		if (response != null && !bTimeOut && listener != null) {
			...
			if (result) {
				...
				listener.onFailure(Constant.MAIL_SNED_FAILURE);
			}
	...
	private boolean getResult(String xml) {
		...
		try {
			...
			e.printStackTrace();
			Logger.print(TAG, " getResult 发送OA邮件结果解析失败 " + e.getMessage());
		}
		return false;
	}
}
