...
public class EmailSendOrReplyPresenter implements EmailSendOrReplyContract.Presenter {
	...
	public void sendEmail(List<String> listReceiver, String senderName, String subject, List<Attachment> listAttachmenet, String content, final boolean isDraft) {
		...
		if (checkTotalAttachmentIsTooBig(listAttachmenet)) {
			view.showErrorMessage(Constant.ATTACHMENT_IS_TOO_BIG);
			return;
		}
	...
	@Override
	public boolean checkAttachmentIsTooBig(String filePath) {
		File file = new File(filePath);
		if (file == null || !file.exists()) {
			return true;
		}
		long length = file.length();
		return Attachment.checkAttachisTooBig(length);
	}
	...
	public boolean checkTotalAttachmentIsTooBig(List<Attachment> list) {
		...
		return Attachment.checkAttachisTooBig(totalLength);
	}

	@Override
	public List<InnerContractsInfo> getInnerReceiver(List<InnerContractsInfo> listCurrent, List<InnerContractsInfo> listSelect) {

		List<InnerContractsInfo> list = new ArrayList<>();

		return list;
	}

	@Override
	public boolean contractsListContainsInputContractsInner(List<InnerContractsInfo> list_selected, List<InnerContractsInfo> list_input) {
		if (list_selected == null || list_selected.size() == 0) {
			return false;
		}
		...
		ArrayList<String> stringSelect = contractsListToStringListInner(list_selected);
		ArrayList<String> stringInput = contractsListToStringListInner(list_input);
		return stringSelect.containsAll(stringInput);
	}
	...
	@Override
	public boolean contractsListNotContainsInputContractsInner(List<InnerContractsInfo> list_selected, List<InnerContractsInfo> list_input) {
		if (list_selected == null || list_selected.size() == 0) {
			return true;
		}
	...
	@Override
	public List<InnerContractsInfo> getNotContainsListInner(List<InnerContractsInfo> list_selected, List<InnerContractsInfo> list_input) {
		ArrayList<InnerContractsInfo> list = new ArrayList<>();
		if (list_selected == null || list_selected.size() == 0) {
			return list;
		}
		...
		ArrayList<InnerContractsInfo> listCommon = new ArrayList<>();
		int size_selected = list_selected.size();
		int size_input = list_input.size();
		for (int i = 0; i < size_selected; i++) {
			String value = list_selected.get(i).getId();
			for (int j = 0; j < size_input; j++) {
				String address = list_input.get(j).getId();
				if (address != null && address.equals(value)) {
					listCommon.add(list_input.get(j));
					break;
				}
